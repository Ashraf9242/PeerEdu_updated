...
type BookingWithTutor = Booking & { tutor: Pick<User, 'name' | 'image'> };
...
export async function UpcomingSessions({ studentId }: { studentId: string }) {
    const { getStudentDashboardData } = await import("../_actions/get-dashboard-data");
    const { upcomingSessions, pendingRequests } = await getStudentDashboardData(studentId);

    return <UpcomingSessionsClient 
        studentId={studentId}
...
export function UpcomingSessionsClient({ upcomingSessions, pendingRequests }: UpcomingSessionsProps) {
                ...
                <TableBody>
                    {allSessions.map((booking) => (
                        <TableRow key={booking.id} className={isPending ? "opacity-50" : ""}>
                            ...
                            <TableCell>{format(new Date(booking.startAt), "EEE, MMM d, yyyy 'at' h:mm a")}</TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem asChild><Link href={`/bookings/${booking.id}`}><Eye className="mr-2 h-4 w-4"/>View Details</Link></DropdownMenuItem>
                                        {booking.meetingLink && <DropdownMenuItem asChild><Link href={booking.meetingLink} target="_blank"><Video className="mr-2 h-4 w-4"/>Join Session</Link></DropdownMenuItem>}
                                        {(booking.status === 'PENDING' || booking.status === 'CONFIRMED') && (
                                            <DropdownMenuItem onClick={() => handleCancel(booking.id)} disabled={isPending}>
